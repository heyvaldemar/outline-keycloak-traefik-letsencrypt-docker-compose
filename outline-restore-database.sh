#!/usr/bin/env bash
# outline-restore-database.sh [backup-file-name]
#
# Replaces the Outline database with one of the dumps the backups service wrote.
#
#   ./outline-restore-database.sh               list and ask
#   ./outline-restore-database.sh <file-name>   restore that one
#
# EVERY PATH, NAME AND CREDENTIAL COMES FROM THE RUNNING BACKUPS CONTAINER.
# The previous version carried outlinedb, outlinedbuser and the backup
...
APP="$(cid "$APP_SERVICE")"; BKP="$(cid backups-outline)"
...
DIR="$(env_of OUTLINE_POSTGRES_BACKUPS_PATH)"; NAME="$(env_of OUTLINE_POSTGRES_BACKUP_NAME)"
DB_NAME="$(env_of OUTLINE_DB_NAME)"; DB_USER="$(env_of OUTLINE_DB_USER)"
...
if ! docker exec "$BKP" sh -c "(set -o pipefail) 2>/dev/null && set -o pipefail; set -eu
    dropdb --force -h postgres-outline -U '$DB_USER' --if-exists '$DB_NAME'
    createdb -h postgres-outline -U '$DB_USER' '$DB_NAME'
    gunzip -c '$DIR/$SELECTED' | psql -q -v ON_ERROR_STOP=1 -h postgres-outline -U '$DB_USER' -d '$DB_NAME' >/dev/null"; then
  echo "error: the restore failed part-way. The database may now be empty: restore another backup before using Outline." >&2
  exit 1
fi
