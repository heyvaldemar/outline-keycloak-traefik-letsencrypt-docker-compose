...
[![OpenSSF Best Practices](https://www.bestpractices.dev/projects/14849/badge)](https://www.bestpractices.dev/projects/14849)
...
Every release from v2.1.0 on carries three files made on GitHub's runner with a short-lived identity and no stored key: `outline-keycloak-traefik-letsencrypt-docker-compose-<tag>.tar.gz`, a `git archive` of exactly the tree the tag points at; `outline-keycloak-traefik-letsencrypt-docker-compose-<tag>.tar.gz.sigstore.json`, a keyless [Sigstore](https://www.sigstore.dev/) signature over it; and `outline-keycloak-traefik-letsencrypt-docker-compose-<tag>.intoto.jsonl`, [SLSA](https://slsa.dev/) build provenance from the SLSA generator. To check them with nothing from this repository trusted:

```bash
cosign verify-blob outline-keycloak-traefik-letsencrypt-docker-compose-<tag>.tar.gz \
  --bundle outline-keycloak-traefik-letsencrypt-docker-compose-<tag>.tar.gz.sigstore.json \
  --certificate-identity-regexp '^https://github.com/heyvaldemar/outline-keycloak-traefik-letsencrypt-docker-compose/' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com

slsa-verifier verify-artifact outline-keycloak-traefik-letsencrypt-docker-compose-<tag>.tar.gz \
  --provenance-path outline-keycloak-traefik-letsencrypt-docker-compose-<tag>.intoto.jsonl \
  --source-uri github.com/heyvaldemar/outline-keycloak-traefik-letsencrypt-docker-compose
```
